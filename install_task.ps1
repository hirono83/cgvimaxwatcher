...
$Trigger = New-ScheduledTaskTrigger -Once -At $Start -RepetitionInterval (New-TimeSpan -Minutes 5) -RepetitionDuration $Duration
...
Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $Trigger -Settings $Settings -Principal $Principal -Description "CGV 용산아이파크몰 오디세이 IMAX $($Config.start_date)~$($Config.end_date) 예매 오픈 감시" -Force | Out-Null

Write-Host "설치 완료: $TaskName"
Write-Host "검사 시간: 하루 24시간, 5분 간격 ($($Config.end_date)까지)"
